...
| 変更 | 内容 |
| --- | --- |
| 大項目4分割 | 起動時の成人／小児選択を廃止し、**内服薬／注射薬／菌名／周術期**を入口にした |
| 集団選択の位置 | 成人／小児は**内服薬・注射薬レーンの第2階層**へ。菌名（菌側の情報）と周術期（体重帯で用量が決まる）では選ばせない。「選択中の集団の用量しか出さない」誤参照防止はそのまま維持 |
| AWaRe絞り込み | 内服薬レーンに Access / Watch / Reserve / その他 の4ボタンを新設（原典 p.74 は内服薬のみが対象のため注射薬には置かない）。注射薬レーンは系統で絞り込む |
...
| 適用範囲の明示 | **当院の院内利用を想定したアプリ**である旨を、初回起動時の確認画面・アプリの説明画面・全画面のフッタに表示。適応外使用・採用薬・使用申請のルール・アンチバイオグラムが当院固有であることを明記 |
| アプリの説明画面 | ヘッダーとホームから常時開ける。使い方、注意事項、アップデートとオフライン利用、患者情報の扱い、免責事項の全文、原典・問い合わせ先 |

### 品質ゲート

```bash
npm run build             # データ検証 → 型チェック → ビルド → SW生成
npm run check:layout      # 幅375/768/1280px × 16画面の配置検査（要 vite preview）
