...
check(/表5.*主な原因食品及び潜伏期間/.test(text), "表5の原因食品・潜伏期間表がある");
check((await page.locator("table.doc tbody tr").count()) === 11, "潜伏期間表は11行");
check(/牡蠣等の二枚貝/.test(text) && /加熱不十分な鶏肉/.test(text), "原因食品を表示する");
check(/抗菌薬を検討する状況/.test(text), "抗菌薬の適応を本文で説明する");
check(/各論：サルモネラ腸炎/.test(text) && /各論：カンピロバクター腸炎/.test(text), "腸炎の各論を表示する");
...
check((await page.locator(".table-organism-link").count()) > 0, "表2の中からアンチバイオグラムへ進める");
await page.locator(".table-organism-link").first().click();
